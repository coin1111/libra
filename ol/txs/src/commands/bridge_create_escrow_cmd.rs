...
impl Runnable for BridgeCreateEscrowCmd {
    fn run(&self) {
        ...
        match bridge_create_escrow(entry_args.save_path) {
            Ok(_) => println!("Success: Create bridge escrow"),
            Err(e) => {
...
pub fn bridge_create_escrow(save_path: Option<PathBuf>) -> Result<TransactionView, TxError> {
    ...
    let script = transaction_builder::encode_bridge_create_escrow_script_function();
